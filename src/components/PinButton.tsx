...
export function PinButton({ item, className = "" }: PinButtonProps) {
  ...
  const itemId = `${item.title}-${item.description}`
    .replace(/\s+/g, "-")
    .toLowerCase();
  ...
  const togglePin = async () => {
    ...
    try {
      const pins: PinItem[] = JSON.parse(
        localStorage.getItem("familyTravelPins") || "[]"
      );
  ...
  return (
    ...
    >
      ...
      <span>{isLoading ? "Bezig..." : isPinned ? "Geprikt" : "Prikken"}</span>
    </button>
  );
}
